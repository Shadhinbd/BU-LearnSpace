    ...
    if ($user) {
        ...
        if (class_exists('PHPMailer\PHPMailer\PHPMailer')) {
            try {
                ...
                $mail->SMTPAuth   = true;
                $mail->Username   = 'together.shadhin@gmail.com'; // ← তোমার Gmail
                $mail->Password   = 'aopqntmjdrqwzptq';    // ← App Password
                $mail->SMTPSecure = 'tls';
                $mail->Port       = 587;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['do_reset'])) {
    ...
    if (strlen($new_pass) < 6) {
        ...
        $stmt = $mysqli->prepare("SELECT id FROM users WHERE reset_token=? AND token_expiry > UTC_TIMESTAMP() AND role='student'");
...
<?php if ($step === 'request'): ?>
  ...
  <p class="subtitle">Enter your student email to receive a reset link</p>
  <div class="steps">
...
<?php elseif ($step === 'reset'): ?>
  ...
  <?php
    if (!$token) {
        ...
        $chk = $mysqli->prepare("SELECT id FROM users WHERE reset_token=? AND token_expiry > UTC_TIMESTAMP() AND role='student'");
